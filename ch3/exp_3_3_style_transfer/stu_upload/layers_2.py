...
class ConvolutionalLayer(object):
    ...
    def col2img(input, height_pad, width_pad, kernel_size, channel, padding, stride):
        ...
        for idxh in range(height):
            for idxw in range(width):
                output[:, :, idxh * stride: idxh * stride + kernel_size,
                idxw * stride: idxw * stride + kernel_size] += input[:, :, :, idxh * width + idxw].reshape(input.shape[0], channel, kernel_size, -1)
...
class MaxPoolingLayer(object):
        ...
        for idxh in range(height):
            for idxw in range(width):
                output[:, :, :, idxh * width + idxw] = input[:, :, idxh * stride: idxh * stride + kernel_size,
                                                       idxw * stride: idxw * stride + kernel_size].reshape(
    ...
    def col2img(input, height_pad, width_pad, kernel_size, channel, padding, stride):
        ...
        for idxh in range(height):
            for idxw in range(width):
                output[:, :, idxh * stride: idxh * stride + kernel_size,
                idxw * stride: idxw * stride + kernel_size] += input[:, :, :, idxh * width + idxw].reshape(
                    input.shape[0], channel, kernel_size, -1)
        return output[:, :, padding: height_pad - padding, padding: width_pad - padding]
    ...
    def forward_speedup(self, input):
        ...
        self.input_col = self.img2col(self.input, self.height_out, self.width_out, self.kernel_size, self.stride).reshape((self.input.shape[0], self.input.shape[1], -1, self.height_out, self.width_out))
    ...
    def backward_speedup(self, top_diff):
        ...
        pool_diff = (self.max_index * top_diff[:, :, np.newaxis, :, :]).reshape(self.input.shape[0], -1,
                                                                                self.height_out * self.width_out)
        bottom_diff = self.col2img(pool_diff, self.input.shape[2], self.input.shape[3], self.kernel_size, self.input.shape[1], 0, self.stride)
