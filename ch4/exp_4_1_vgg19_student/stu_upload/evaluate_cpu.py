...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            # TODO: 执行全连接层计算，计算结果存入 current (1, 7, 7, 512)
            kernels, bias = weights[i][0][0][0][0]  # (7, 7, 512, 4096), (1, 4096)
            # print(current.shape, '======================1')
            # print('kernels shape: ', kernels.shape, '-----1')
            ...
            # print(current.shape, '======================2')
            # print('kernels shape: ', kernels.shape, '-----2')
            # print('bias shape: ', bias.shape, '-----2')
            kernels, bias = weights[i][0][0][0][0]
            kernels = tf.reshape(kernels, [kernels.shape[0] * kernels.shape[1] * kernels.shape[2], -1])
            current = tf.nn.bias_add(tf.matmul(current, kernels), bias.flatten())
        elif name == 'fc8':
            # print(current.shape, '======================3')
            # print('kernels shape: ', kernels.shape, '-----3')
...
def load_image(path):
    ...
    image = scipy.misc.imresize(scipy.misc.imread(path), size=(224, 224, 3))
